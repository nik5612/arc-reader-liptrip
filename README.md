...
Чтобы вытащить всё из всех файлов, что находятся на уровень ниже, нужна команда:
./extract_all_arcs.sh &> log.txt

Чтобы пересобрать сам экстрактор, то main clean && main

Чтобы воспользоваться нерабочей функцией по сборке обратно, команда:
./repack_all.sh
